...
class VerticaDialect(PGDialect):
    ...
    def is_disconnect(self, e, connection, cursor):
        return isinstance(e, self.dbapi.Error) and connection is not None and connection.closed()
    ...
    def has_schema(self, connection, schema):
        query = ("SELECT EXISTS (SELECT schema_name FROM v_catalog.schemata " "WHERE schema_name='%s')") % (schema)
    ...
    def has_table(self, connection, table_name, schema=None):
        ...
        query = ("SELECT EXISTS (" "SELECT table_name FROM v_catalog.all_tables " "WHERE schema_name='%s' AND " "table_name='%s'" ")") % (schema, table_name)
        rs = connection.execute(text(query))
        return bool(rs.scalar())
    ...
    def has_sequence(self, connection, sequence_name, schema=None):
        if schema is None:
            schema = self._get_default_schema_name(connection)
        query = ("SELECT EXISTS (" "SELECT sequence_name FROM v_catalog.sequences " "WHERE sequence_schema='%s' AND " "sequence_name='%s'" ")") % (schema, sequence_name)
        rs = connection.execute(text(query))
        return bool(rs.scalar())

    def has_type(self, connection, type_name, schema=None):
        query = ("SELECT EXISTS (" "SELECT type_name FROM v_catalog.types " "WHERE type_name='%s'" ")") % (type_name)
        rs = connection.execute(text(query))
        return bool(rs.scalar())

    def _get_server_version_info(self, connection):
        v = connection.scalar(text("select version()"))
        m = re.match(".*Vertica Analytic Database " "v(\d+)\.(\d+)\.(\d)+.*", v)
        if not m:
    ...
    def get_table_comment(self, connection, table_name, schema=None, **kw):
        schema_conditional = "" if schema is None else "AND object_schema = '{schema}'".format(schema=schema)
        query = """
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        schema_conditional = "" if schema is None else "AND table_schema = '{schema}'".format(schema=schema)
        ...
        for row in list(connection.execute(text(column_select))):
            sequence_info = (
                ...
                {schema_conditional}
                """.format(table_name=table_name, schema_conditional=("" if schema is None else "AND sequence_schema = '{schema}'".format(schema=schema)))
                    )
                ).first()
                if row.is_identity
                else None
            ...
            colobj = self._get_column_info(row.column_name, row.data_type, row.is_nullable, row.column_default, row.is_identity, (row.column_name in primary_key_columns), sequence_info)
            if colobj:
    ...
    def _get_column_info(self, name, data_type, is_nullable, default, is_identity, is_primary_key, sequence):
        ...
        column_info = {"name": name, "type": typeobj, "nullable": is_nullable, "default": default, "primary_key": (is_primary_key or is_identity)}
        if is_identity:
    ...
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):
        query = (
            "SELECT constraint_id, constraint_name, column_name FROM v_catalog.constraint_columns \n\
                 WHERE table_name = '"
            + table_name
            + "'"
        )
        ...
        for key in result_dict.keys():
            result.append({"name": key, "column_names": result_dict[key]})
    ...
    def get_check_constraints(self, connection, table_name, schema=None, **kw):
        ...
        """.format(table_name=table_name, schema_clause=("" if schema is None else "AND i.table_schema ='{schema}'".format(schema=schema)))

        return [{"name": name, "sqltext": src[1:-1]} for name, src in connection.execute(text(query)).fetchall()]
